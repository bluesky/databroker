...
import uuid
from metadataclient.api import insert_event, insert_descriptor, find_events
import numpy as np
...
def run(run_start=None, sleep=0):
    ...
    data_keys1 = {'point_det': dict(source='PV:ES:PointDet',
                                    dtype='number', shape=point_det_data.shape[0])}
    data_keys2 = {'Tsam': dict(source='PV:ES:Tsam', dtype='number', shape=1),
                  'Troom': dict(source='PV:ES:Troom', dtype='number', shape=1)}

...
def create_file_link(file_id, spec, file_path, custom=None):
    ...
    try:
        flink_id = db['base'].insert(f_base.bson, wtimeout=100, write_concern={'w': 1})
...
def create_file_attributes(shape, dtype, **kwargs):
    ...
    try:
        f_attr_id = db['public'].insert(f_attr.bson, wtimeout=100, write_concern={'w': 1})
    except OperationFailure:
...
def create_event_list(event_id, file_id, event_list_custom=None):
    ...
    try:
        event_list_id = db['event_list'].insert(event_list.bson, wtimeout=100, write_concern={'w': 1})
    except OperationFailure:

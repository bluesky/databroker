...
from fileStore.database.databaseInit import db
...
def create_file_link(file_id, spec, file_path, custom=None):
    ...
    try:
        flink_id = db['file_base'].insert(f_base.bsonify(),
                                          wtimeout=100, write_concern={'w': 1})
    except OperationFailure:
        raise PyMongoError(_connection_err_msg.format("File link"))
...
def create_file_attributes(shape, dtype, **kwargs):
    ...
    try:
        f_attr_id = db['file_attributes'].insert(
            f_attr.bsonify(), wtimeout=100, write_concern={'w': 1})
    except OperationFailure:
        raise PyMongoError(_connection_err_msg.format("File attributes"))
...
def create_event_list(event_id, file_id, event_list_custom=None):
    ...
    try:
        event_list_id = db['event_list'].insert(
            event_list.bsonify(), wtimeout=100, write_concern={'w': 1})
    except OperationFailure:
        raise PyMongoError(_connection_err_msg.format("File event list"))
